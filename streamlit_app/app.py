...
if not logger.handlers:
    handler = RotatingFileHandler(LOG_FILE, maxBytes=5 * 1024 * 1024, backupCount=3)  # 5 MB на файл, до 3 резервных копий
...
# Боковая панель
st.sidebar.markdown('<h2 class="subheader">Дополнительный функционал:</h2>', unsafe_allow_html=True)
...
if st.sidebar.button("⚙️ Дообучить модель YOLO на двух картинках ))"):
    with st.spinner("Дообучение модели, пожалуйста подождите..."):
        try:
            response = requests.post(f"{BASE_URL}/fit", json={"model_id": 'detect'})
            if response.status_code == 200:
                message = response.json().get("message", "Модель успешно дообучена!")
            ...
            st.sidebar.error(f"⚠️ Произошла ошибка при дообучении модели: {str(e)}")
...
if st.sidebar.button("🛠️ Проверить состояние сервиса"):
    try:
        ...
        if response.status_code == 200:
            st.sidebar.success(f"✅ Сервис работает: {response.json().get('status', 'OK')}")
            logger.info("Сервис доступен и работает.")
        else:
            st.sidebar.error(f"❌ Сервис недоступен. Код ответа: {response.status_code}")
            logger.error(f"Проблема с сервисом: код ответа {response.status_code}")
    except Exception as e:
...
if st.button("Загрузить модель"):
    with st.spinner(f"Загрузка модели {selected_model}, пожалуйста подождите..."):
        try:
            response = requests.post(f"{BASE_URL}/load", params={"model_id": selected_model})
            if response.status_code == 200:
                ...
                logger.error(f"Ошибка загрузки модели {selected_model}: {response.text}")
        except Exception as e:
...
st.markdown('<h2 class="subheader">Загрузка изображения для предсказания</h2>', unsafe_allow_html=True)
uploaded_files = st.file_uploader("Выберите изображение/изображения", type=["jpg", "jpeg", "png"], accept_multiple_files=True)
...
if uploaded_files and st.button("🔍 Сделать предсказание"):
    ...
    try:
        response = requests.post(f"{BASE_URL}/predict", params={"model_id": selected_model}, files=files)
        if response.status_code == 200:
            ...
            if not predictions:
                ...
                st.markdown('<h2 class="subheader">Результаты предсказания:</h2>', unsafe_allow_html=True)
